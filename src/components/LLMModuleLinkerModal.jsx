...
// ---------------------- AUTH FIX + raw OpenAI call ----------------------
// NOTE: Use Bearer; your earlier code likely caused 401s.
...
`.trim();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to parse LLM JSON for linker:", err, { raw, cleaned });
  ...
  const applySelected = async () => {
    ...
    try {
      ...
      if (skipped > 0) {
        console.warn(`Skipped ${skipped} intra-module link(s).`);
      }
      onClose?.();
    } catch (e) {
