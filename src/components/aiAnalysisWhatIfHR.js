...
export async function generateWhatIfSeedSheet({ sheets, setFolders, currentFolder }) {
  console.log("📥 Entered generateWhatIfSeedSheet (HR with AI scenarios)");
  ...
  console.log(`✅ Generated HR What-If seed sheet (15 AI scenarios) → ${key}`);
...
export async function populateWhatIfScenariosWithLLM({
  ...
  console.log("📊 populateWhatIfScenariosWithLLM (HR) called");

  const sheetKey = `What-If Scenarios${NS}`;
  const sheet = sheets[sheetKey];
  if (!sheet || sheet.length < 2) {
    console.warn(`⚠️ ${sheetKey} sheet is missing or empty.`);
    return;
  }
...
`.trim();
  ...
  console.log("✅ Finished populating HR What-If scenario columns");
...
export async function generateWhatIfCausalFactorsSheet({
  ...
  console.log("🧠 generateWhatIfCausalFactorsSheet (HR) called");
  ...
  console.log(`✅ Created HR Causal Factors sheet: ${rows.length - 1} rows`);
  return updatedSheets;
}
...
export async function generateMitigationStrategiesSheet({
  ...
  if (!causalSheet || causalSheet.length < 2) {
    console.warn(`⚠️ '${causalKey}' sheet is missing or empty.`);
    return;
  }
...
export async function generateSystemRequirementsSheet({
  ...
  if (!mitigationSheet || mitigationSheet.length < 2) {
    console.warn(`⚠️ '${mitigationKey}' sheet is missing or empty.`);
    return;
  }
...
export async function generateBatchedRequirementsSheet({
  ...
  if (!systemReqs || systemReqs.length < 2) {
    console.warn(`⚠️ '${systemReqKey}' sheet is missing or empty.`);
    return;
  }
...
export async function generateLossMappingsSheet({
  ...
  if (!hazardSheet || hazardSheet.length < 2) {
    console.warn(`⚠️ '${hazardKey}' sheet is missing or empty.`);
    return;
  }
...
export async function generateSummarySheetFromMappings({
  ...
  console.log("🔍 [Summary Gen - HR] Checking required sheets...");
  ...
  console.log("🔍 [Summary Gen - HR] Sheet presence check:", sheetStatus);

  const missingSheets = Object.entries(sheetStatus).filter(([, ok]) => !ok).map(([k]) => k);
  if (missingSheets.length) {
    console.warn(`⚠️ [Summary Gen] Missing/insufficient sheets: ${missingSheets.join(", ")}`);
    return;
  }
