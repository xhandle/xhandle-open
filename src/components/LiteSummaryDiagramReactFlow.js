...
const DiagramBody = forwardRef(function DiagramBody(
  ...
  const selectedGroupedNodeIds = useMemo(
    () =>
      selectedNodeIds.filter((id) => {
        const node = nodes.find((n) => n.id === id);
        return node && node.type !== 'groupBox' && !!node.parentNode;
      }),
...
useEffect(() => {
  ...
  const addGroupBox = useCallback(() => {
    const id = `g:${cryptoId()}`;
    const position = nearestFreePosition(seedPosition(0), getNodes().filter((n) => !n.parentNode));
    setGroupBoxes((currentBoxes) => {
      const label = `Group ${currentBoxes.length + 1}`;
      const next = [
        ...currentBoxes,
        { id, label, position, width: GROUP.w, height: GROUP.h, brandColor: BRAND.purple },
      ];
      persistGroupsSoon(next);
      return next;
    });
  }, [getNodes, persistGroupsSoon]);
...
<button
  onClick={() => {
    ...
    posRef.current.set(rfId, { position, parentId: null });
    persistSoon();
  }}
  style={{
...
<button
  ...
  style={{
    background: '#fff',
    color: BRAND.dark,
    border: `1px solid ${rgba(BRAND.yellow, 0.55)}`,
    borderRadius: 8,
    padding: '8px 12px',
    fontWeight: 700,
    boxShadow: '0 6px 16px rgba(243,182,63,0.16)',
    cursor: 'pointer',
  }}
>
  📝 Note
...
<button
  onClick={() => ungroupNodes(selectedGroupedNodeIds)}
  disabled={!selectedGroupedNodeIds.length}
  style={{
    background: selectedGroupedNodeIds.length ? '#fff' : 'rgba(255,255,255,0.6)',
    color: selectedGroupedNodeIds.length ? BRAND.purple : 'rgba(122,55,255,0.6)',
    border: `1px solid ${rgba(BRAND.purple, selectedGroupedNodeIds.length ? 0.45 : 0.2)}`,
    borderRadius: 8,
    padding: '8px 12px',
    fontWeight: 700,
    boxShadow: selectedGroupedNodeIds.length ? '0 6px 16px rgba(122,55,255,0.12)' : 'none',
    cursor: selectedGroupedNodeIds.length ? 'pointer' : 'not-allowed',
  }}
  title={selectedGroupedNodeIds.length ? 'Remove selected nodes from their group' : 'Select grouped nodes to ungroup them'}
>
  ⤴ Ungroup
</button>
        ...
        <button
          ...
          title="Export diagram data as JSON"
        >
          📤 Export .JSON
        </button>
      </div>
