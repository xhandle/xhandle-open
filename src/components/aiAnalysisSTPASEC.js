...
async function throttleLLMCall(prompt) {
  ...
  console.log(`🧮 Prompt length: ${prompt.length}, ~Tokens: ${estimatedTokens}`);
  ...
  while (totalTokensUsed + estimatedTokens > MAX_TOKENS_PER_MINUTE) {
    console.log(`⏳ Waiting for token budget to reset... (${totalTokensUsed}/${MAX_TOKENS_PER_MINUTE})`);
    await sleep(300);
  }
  ...
  if (timeSinceLastCall < MIN_DELAY_MS) {
    ...
    console.log(`⏱️ Waiting ${waitTime}ms to respect 10 RPM limit`);
...
async function fetchWithRetry(prompt, retries = 3, delay = 300) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt < retries) {
        await sleep(delay * attempt); // grows: 300ms, 600ms, 900ms
      }
    }
  }
...
export async function generateVulnerableControlActionsSheet_STPASEC({ sheets, setFolders, currentFolder }) {
  console.log("📥 Entered generateVulnerableControlActionsSheet_STPASEC");
  ...
  console.log("✅ Generated VCA sheet (STPA-SEC) with decomposition context");
...
export async function populateVCAThreatColumnsWithLLM_STPASEC({
  ...
  console.log("📊 populateVCAThreatColumnsWithLLM_STPASEC called");

  const sheet = sheets["Vulnerable Control Actions (STPA-SEC)"];
  if (!sheet || sheet.length < 2) {
    console.warn("⚠️ 'Vulnerable Control Actions (STPA-SEC)' is missing or empty.");
    return;
  }
...
- Keep it specific and security-focused (integrity, availability, confidentiality, routing/recipient errors, timing/order issues).
      ...
      try {
        ...
        console.error(`LLM error for row ${r}, column ${c}`, err);
        resp = "(LLM error)";
      }
...
export async function generateSecurityControlsSheet_STPASEC({
  ...
  if (!ts || ts.length < 2) {
    console.warn("⚠️ 'Threat Scenarios' sheet is missing or empty.");
    return;
  }
...
`.trim();
    ...
    try {
      ...
      console.error("LLM error for security control:", err);
...
export async function generateSystemSecurityRequirementsSheet_STPASEC({
  ...
  if (!controls || controls.length < 2) {
    console.warn("⚠️ 'Security Controls' sheet is missing or empty.");
    return;
  }
...
`.trim();
    ...
    try {
      ...
      console.error("LLM error for security requirement:", err);
...
`.trim();
    ...
    try {
      ...
      console.error("LLM error generating security category:", err);
      cat = "(error)";
    }
...
export async function generateSecurityLossMappingsSheet_STPASEC({
  ...
  if (!secCat || secCat.length < 2) {
    console.warn("⚠️ 'Security Category Mappings' sheet is missing or empty.");
    return;
  }
...
export async function generateSecuritySummarySheet_STPASEC({
  ...
  if (Object.values(ok).some(v => !v)) {
    console.warn("⚠️ Missing sheets for Security Summary (detail):", {
    ...
    console.warn("⚠️ Missing sheets for Security Summary:", ok);
    return;
  }
