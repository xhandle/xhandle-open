/**
 * xHandle: vn vpanel agent workspace.
...
export default function VnVPanel({ performTask, activeProjectId }) {
  const caps = capabilitySchema[AgentKinds.VNV];

  const runTask = async (task) => {
    const p = (task.prompt || "").toLowerCase();
    const type = p.includes("coverage") || p.includes("gap")
      ? "vnv.compute-coverage"
      : "vnv.generate-tests";
    await dispatchAgentAction(AgentKinds.VNV, type, { projectId: activeProjectId, userPrompt: task.payload?.userPrompt }, performTask);
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <Schema caps={caps} />
      <AgentTaskComposer kind={AgentKinds.VNV} onCreated={()=>{}} />
      <AgentTaskList kind={AgentKinds.VNV} onRun={runTask} />
    </div>
  );
}
