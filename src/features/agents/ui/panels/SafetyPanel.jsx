/**
 * xHandle: safety panel agent workspace.
...
export default function SafetyPanel({ performTask, activeProjectId }) {
  const caps = capabilitySchema[AgentKinds.SAFETY];
  const [methodDefault] = useState("FMEA");

  const runTask = async (task) => {
    const p = (task.prompt || "").toLowerCase();
    if (p.includes("mitigation")) {
      await dispatchAgentAction(AgentKinds.SAFETY, "safety.generate-mitigations", { projectId: activeProjectId, userPrompt: task.payload?.userPrompt }, performTask);
    } else {
      // detect method in prompt, fallback to default
      const method = /stpa/.test(p) ? "STPA" : /what[\s-]?if/.test(p) ? "WhatIf" : methodDefault;
      await dispatchAgentAction(AgentKinds.SAFETY, "safety.run-analysis", { projectId: activeProjectId, method, userPrompt: task.payload?.userPrompt }, performTask);
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <Schema caps={caps} />
      <AgentTaskComposer kind={AgentKinds.SAFETY} onCreated={()=>{}} />
      <AgentTaskList kind={AgentKinds.SAFETY} onRun={runTask} />
    </div>
  );
}
